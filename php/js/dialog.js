...
window.onload=function(){
	var cssText = "html,body{width:100%;height:100%;margin:0;padding:0;}#mh_layer{background-color:#000;position:fixed;left:0;top:0;width:100%;height:100%;z-index:999999;display:none;background-color:rgba(0,0,0,0.6);}#mh_dialog{width:200px;height:100px;text-align:center;border-radius:8px;position:fixed;left:50%;top:50%;margin-left:-106px;margin-top:-56px;z-index:10000000;background-color:#fff;background-repeat:no-repeat;line-height:140px;font-size:14px;font-weight:bold;display:none;letter-spacing:1px;}.mh_success{background-image:url('../images/success.png');color:#1fce11;background-size:32px 32px;background-position:center 20px;border:6px #1fce11 solid;}.mh_loading{background-image:url('../images/loading.gif');color:#333333;background-size:50px 50px;background-position:center 15px;border:6px #333333 solid;}.mh_warning{background-image:url('../images/warning.png');color:#F90;background-size:32px 32px;background-position:center 20px;border:6px #F90 solid;}.mh_error{background-image:url('../images/error.png');color:#F00;background-size:32px 32px;background-position:center 20px;border:6px #F00 solid;}";
	//初始化插件样式
	createStyle(cssText);
	//创建遮罩层
	var layer_div = document.createElement("div"); 
	layer_div.setAttribute("id","mh_layer"); 
	document.body.appendChild(layer_div); 
	//创建对话框层
	var dialog_div = document.createElement("div"); 
	dialog_div.setAttribute("id","mh_dialog"); 
 ...
 * @return
 */
function dialogShow(className,content,timeout,flag,url){
	//获取遮罩层对象
 ...
 * @return
 */
function dialogClose(url){
	//获取遮罩层对象
	var layer = document.getElementById("mh_layer");
	//获取对话框层对象
	var dialog = document.getElementById("mh_dialog");
	url = url || "";
